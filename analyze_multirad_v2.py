#!/usr/bin/env python3
"""Análise completa do log MultiRad - calcular tempos reais de execução"""

import re
from datetime import datetime

# Log completo fornecido pelo usuário
log_full = """2026-02-25__08:43:49__734007 INFO: i0[204417] started r1 >>>>>>>>>> r27
...
# Filtrar durações positivas
valid_runs = [r for r in runs if r['duration'] > 0]
...
print("=" * 80)
print()
print(f"Execuções com pares started/exited: {len(runs)}")
print(f"Execuções com duração válida (>0):  {len(valid_runs)}")
...
print("=" * 80)
print("TEMPOS DE EXECUÇÃO (durações válidas)")
print("=" * 80)
for r in sorted(valid_runs, key=lambda x: x['run']):
    print(f"Run {r['run']:3d} [i{r['instance']}] [pid {r['pid']:>5}]: {r['duration']:8.2f}s | {r['status']}")
