# MLflow Python API Reference - Resumo
...
Wrapper around `mlflow.entities.Run` para usar com context manager (`with`):

```python
with mlflow.start_run() as run:
    mlflow.log_param("my", "param")
    mlflow.log_metric("score", 100)
# Run terminado automaticamente ao final do bloco
```

### ActiveModel (MLflow 3)

Wrapper around `mlflow.entities.LoggedModel` para gerenciar modelos logados independentemente de runs.
...
# Por ID (usa environment variable)
import os
...
## Workspace Management (Experimental)
...
workspace = mlflow.create_workspace(
    name="my-workspace",
    description="ML experiments workspace",
    default_artifact_root="s3://bucket/"
)

# Obter workspace
workspace = mlflow.get_workspace("my-workspace")

# Deletar workspace
mlflow.delete_workspace("my-workspace", mode="CASCADE")
```
...
9. **Workspaces** - Organização de experimentos (experimental)
